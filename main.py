...
# Author: Javid Uddin, UIN: 656128438
# 
...
def command1(dbConn):
    ...
    # 'like' keyword is used for SQL wildcards and partial data matching
    dbCursor.execute("Select Station_ID, Station_Name from Stations where Station_Name like (?) order by Station_Name asc;", [inp])
    result = dbCursor.fetchall()
    if(len(result) == 0):
        print("**No stations found...")
    else:
        for row in result:
            # Dissect the row tupel into individual elements
...
def command2(dbConn):
    ...
    # requires exact match (=), joins Stations table to Ridership table by matching the Station ID's
    dbCursor.execute("Select sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?);", [inp]) # total Ridership stats for the indputted station name
    totalRidership = dbCursor.fetchone()
    if(totalRidership[0] == None):
        print("**No data found...")
    else:
        # Total ridership during the week
        dbCursor.execute("Select sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?) and Type_of_Day = 'W';", [inp]) # W denotes Weekdays
        weekdayRidership = dbCursor.fetchone()
        # Total ridership during Saturdays
        dbCursor.execute("Select sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?) and Type_of_Day = 'A';", [inp]) # A denotes Saturdays
        saturdayRidership = dbCursor.fetchone()
        # Total ridership for Sundays/Holidays
        dbCursor.execute("Select sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?) and Type_of_Day = 'U';", [inp]) # U denotes Sundays/Holidays
        sundayHolidayRidership = dbCursor.fetchone()
        print(f"Percentage of ridership for the {inp} station: ")
        print("  Weekday ridership:", f"{weekdayRidership[0]:,}", f"({weekdayRidership[0]/totalRidership[0] * 100:.2f}%)")
        print("  Saturday ridership:", f"{saturdayRidership[0]:,}", f"({saturdayRidership[0]/totalRidership[0] * 100:.2f}%)")
        print("  Sunday/holiday ridership:", f"{sundayHolidayRidership[0]:,}", f"({sundayHolidayRidership[0]/totalRidership[0] * 100:.2f}%)")
...
def command3(dbConn):
    dbCursor = dbConn.cursor()
    dbCursor.execute("Select sum(Num_Riders) from Ridership where Type_of_Day = 'W';") # total Ridership stats for weekdays
    totalWeekdayRidership = dbCursor.fetchone()
    # Displays percentages of each stations ridership on weekdays
    dbCursor.execute("Select Station_Name, sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Type_of_Day = 'W' group by Station_Name order by sum(Num_Riders) desc;") # total Ridership stats for the indputted station name
...
def command4(dbConn):
    ...
    for row in results: # compare all valid colors retrieved by SQL with user input
    ...
    if(not validColor):
        ...
        inputDir.lower()
        # Translates cardinal directions into their short form to match table data
        if(inputDir == 'north'):
            inputDir = 'n'
        elif(inputDir == 'south'):
            inputDir = 's'
            ...
            inputDir = 'e'
        # Multi-table combination to utilize station name and retrieve stop name, direction, and handicap accessibility status. NOT case sensitive
        dbCursor.execute("Select Stop_Name, Direction, ADA from Stops join StopDetails on Stops.Stop_ID = StopDetails.Stop_ID join Lines on StopDetails.Line_ID = Lines.Line_ID where Color = (?) COLLATE NOCASE and Direction = (?) COLLATE NOCASE order by Stop_Name asc;", [inputColor, inputDir])
        ...
        if(len(resultStops) == 0):
            ...
            for row in resultStops:
                ...
                # Convert binary status to string
                if(ada == 1):
...
def command5(dbConn):
    dbCursor = dbConn.cursor()
    # Each color line is grouped by direction, and ordered by ascending color name then ascending direction (alphabetically)
    dbCursor.execute("Select Color, Direction, Count(*) from Stops join StopDetails on Stops.Stop_ID = StopDetails.Stop_ID join Lines on StopDetails.Line_ID = Lines.Line_ID group by Direction, Color order by Color asc, Direction asc;")
    results = dbCursor.fetchall()
    dbCursor.execute("Select Count(*) from Stops;")
...
def command6(dbConn):
    ...
    # Allows for wildcards / partial names
    ...
    # Multiple matches (force reselection in order to calculate data on only one station)
    elif(len(stationResults) > 1):
        print("**Multiple stations found...")
    else:
        stationName = stationResults[0][0]
        # Groups station data by each year, selects year and the summation of riders for that year for the specific station
        dbCursor.execute("Select distinct strftime('%Y', Ride_Date) as Year, sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?) group by Year;", [stationName])
        ...
        askPlot = input("\nPlot? (y/n) ")
        if(askPlot.lower() == 'y'):
            # x and y vectors to plot
            x = []
            y = []
            year = 1
            # append retrieved information into the x&y vectors
            for row in yearlyRidership:
                year, numRiders = row
                x.append(year)
                y.append(numRiders)
...
def command7(dbConn):
    ...
    if(len(stationResults) == 0):
        print("**No station found...")
    # Only one station can be selected per query
    elif(len(stationResults) > 1):
        print("**Multiple stations found...")
    else:
        stationName = stationResults[0][0]
        inpYear = input("Enter a year: ")
        # Gets data for total ridership for each month in a year for a specific station
        dbCursor.execute("Select strftime('%m', Ride_Date), sum(Num_Riders) from Ridership join Stations on Ridership.Station_ID = Stations.Station_ID where Station_Name = (?) and strftime('%Y', Ride_Date) = (?) group by strftime('%m', Ride_Date);", [stationName, inpYear])
        ...
        askPlot = input("\nPlot? (y/n) ")
        # Plots the trend between each month for the specified year and station
        if(askPlot.lower() == 'y'):
            x = []
            y = []
            month = 1
...
def command8(dbConn):
    ...
    if(len(stationResults) == 0):
        ...
        stationID1 = stationResults[0][0] # station 1 id
        stationName1 = stationResults[0][1] # station 1 name (for reference/printing)
        ...
        if(len(stationResults) == 0):
            ...
            stationID2 = stationResults[0][0] # station 1 id
            stationName2 = stationResults[0][1] # station 2 name

            print(f"Station 1: {stationID1} {stationName1}")
            # Gets total ridership for each day of the specified year for the first specified station, checks station ID and year match
            dbCursor.execute("Select date(Ride_Date), Num_Riders from Ridership where Station_ID = (?) and strftime('%Y', Ride_Date) = (?) order by Ride_Date asc;", [stationID1, inpYear])
            resultStation1 = dbCursor.fetchall()
             # Prints first 5 entries (01/01 - 01/05) for first station
            for row in resultStation1[:5]:
                rideDate, numRiders = row
                print(f"{rideDate} {numRiders}")
            # Prints last 5 entries (12/27 - 12/31) for first station
            for row in resultStation1[-5:]:
                rideDate, numRiders = row
                print(f"{rideDate} {numRiders}")

            print(f"Station 2: {stationID2} {stationName2}")
            # Gets total ridership for each day of the specified year for the second specified station, checks station ID and year match
            dbCursor.execute("Select date(Ride_Date), Num_Riders from Ridership where Station_ID = (?) and strftime('%Y', Ride_Date) = (?) order by Ride_Date asc;", [stationID2, inpYear])
            resultStation2 = dbCursor.fetchall()
            # Prints first 5 entries (01/01 - 01/05) for second station
            for row in resultStation2[:5]:
                rideDate, numRiders = row
                print(f"{rideDate} {numRiders}")
            # Prints last 5 entries (12/27 - 12/31) for second station
            for row in resultStation2[-5:]:
                rideDate, numRiders = row
                print(f"{rideDate} {numRiders}")
            # Plots data for both trends to allow for a comparative analysis
            askPlot = input("\nPlot? (y/n) ")
            if(askPlot.lower() == 'y'):
                # station 1
                x1 = []
...
def command9(dbConn):
    ...
    if(inpLat >= 40 and inpLat <= 43):
        ...
        if(inpLong <= -87 and inpLong >= -88):
            ...
            dbCursor.execute("Select distinct Station_Name, Latitude, Longitude from Stops join Stations on Stops.Station_ID = Stations.Station_ID where Latitude between (?) and (?) and Longitude between (?) and (?) order by Station_Name asc;", [latLower, latUpper, longLower, longUpper])
            results = dbCursor.fetchall()
            if(len(results) > 0):
                ...
                if(askPlot.lower() == 'y'):
                    ...
                    for row in results:
                        ...
                        x.append(longitude)
                        y.append(latitude)
                    # Plot on top of chicago OSM
                    image = plt.imread("chicago.png")
                    # Dimensions of map boundaries
                    xydims = [-87.9277, -87.5569, 41.7012, 42.0868] 
                    plt.imshow(image, extent=xydims)
                    plt.title("Stations Near You")
                    # Make plotted points visible
                    plt.plot(x, y, marker='o', linestyle='', color='blue', label='stations')
                    # Annotate plotted points with label (Station name)
                    for row in results:
                        stationName, latitude, longitude = row
...
cont = True # boolean for whether or not to continue the program
while(cont):
